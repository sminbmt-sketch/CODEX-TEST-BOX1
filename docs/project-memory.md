...
- 2026-07-16: Tanium Inventory was extended beyond endpoint basics. Endpoint snapshots now store installed software, optional process sensor readings, service details, and SBOM-based Comply CVE findings in JSON fields. The inventory UI shows per-endpoint counts and previews for Installed Software, Processes, and SBOM Findings. Optional process/SBOM collection failures do not block the core endpoint/software sync.
