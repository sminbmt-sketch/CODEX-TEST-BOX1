...
- 2026-08-14: HOT Topic LLM processing also evaluates candidate keywords that are too generic to be useful Topics. When enabled in Settings, validated LLM-suggested generic keywords are automatically appended to the HOT Topic exclude filter and future dashboard snapshots omit them.
- 2026-08-14: HOT Topic auto-exclude is intentionally conservative: LLM suggestions are accepted only when they match current candidate keywords and pass the generic allowlist/protected product-vendor checks. Specific vendors, products, technologies, events, and short meaningful security acronyms should not be auto-filtered.
- 2026-08-14: HOT Topic cards no longer display article view counts because current collectors often cannot populate reliable view metrics. LLM HOT Topic output is compacted for low max-token settings, and missing per-topic descriptions are backfilled with a related top article title.
- 2026-08-14: Settings UI changed from a long stacked-card page to a two-pane layout. The left Settings section list controls which configuration UI appears on the right, with Quick Actions separated from detailed settings.
- 2026-08-14: Settings Quick Actions remains in the sticky top area, while the left Settings list controls only detailed configuration sections. Hidden sections must use an explicit `[hidden]` CSS rule so source-management cards do not leak into every Settings section.
- 2026-08-14: Dashboard visual style was updated toward a modern dark SOC/Bento layout: softer layered cards, four-metric row, CVE left, HOT Topic under CVE, Security News full-height right panel, and denser HOT Topic tiles with LLM descriptions.
...
- 2026-07-20: Investigation now uses an LLM-planned workflow when an LLM provider is enabled: source URL text is focused to vulnerability/product/version-relevant lines, the LLM creates compact `investigation_keywords`, backend allowlisted Tanium read-only inventory evidence is collected, and the LLM judges endpoint impact from that evidence. If LLM planning or judgement fails, the workflow falls back to rules and records planner/judge errors. NGINX fixed-version articles now extract NGINX/NGINX Plus product/version rules, avoid treating version `37.0.3.1` as an IP IOC, and classify endpoints with matching process/service/software evidence as review candidates.
- 2026-07-22: Investigation target selection now includes a separate `KISA 보안공지` tab. The tab loads Article records through the existing `category=kisa` article filter and runs investigations through the existing news Article intelligence path, preserving backend `source_type=news` compatibility while separating KISA notices from general Security News in the operator workflow.
- 2026-07-22: Investigation source handling no longer uses lossy keyword/length focusing before LLM planning. Fetched source text is capped only by a 30,000-character investigation limit, HTML tables are preserved as row-level `TABLE_ROW` text, and affected product rows now retain CVE, affected version, and fixed version conditions. Investigation LLM calls use at least 2048 output tokens, up to 4096, while normal summary behavior remains unchanged.
- 2026-07-27: Tanium Inventory expansion scope added. Endpoint snapshots should store extra read-only Tanium sensor data for OS kernel/build information, chassis type, virtualization status, MAC address via the Mac Address sensor, and open ports. The Inventory table keeps the existing core columns and exposes these extra values through an `ETC` detail button/modal.
- 2026-07-27: Linux/macOS/Solaris kernel version collection uses a read-only Tanium REST question fallback (`Get Computer Name and Kernel Version from all machines`) because the `Kernel Version` sensor exists but is not registered with Tanium Data Service for Gateway `sensorReadings`. Results are matched back to endpoint snapshots by hostname and stored in Inventory ETC as `Kernel Version`; Linux/Unix-like endpoints with no returned value still show a Kernel Version row with a Tanium no-result status.
- 2026-07-30: Settings Data Management reset must delete dependent Investigation records before source data. The reset order is `investigation_runs`, `news_intelligence`, `detections`, then CVE/news/inventory tables so `articles` and `vulnerabilities` are not blocked by foreign-key references.
- 2026-07-30: Investigation UI keeps the primary `조사 실행` action in the top page-title action area instead of below the target list. When an investigation run completes, the UI automatically scrolls to the investigation result card.
- 2026-08-03: CVE and Security News list pages expose a top-level `조사 실행` action that sends the selected/current visible item to Investigation and runs the Tanium read-only investigation immediately. Investigation target selection supports search and adds an `E-mail` segmented tab. A new Email menu stores sender-filtered mail items, lists them with search/sort/pagination, and sends selected mail content into the same Investigation flow without summary actions.
- 2026-08-03: CVE, Security News, and Email `조사 실행` now uses an explicit investigation-selection mode. Pressing `조사 실행` first reveals single-select radio controls on visible cards, and only one target can be selected before running `선택 항목 조사`. Summary multi-select remains separate from investigation single-select.
- 2026-08-03: Added normalized IOC/Entity storage for Investigation output. `intelligence_entities` stores affected products, software, versions, services, processes, OS values, and CVEs; `intelligence_iocs` stores IP/domain/hash/file/port indicators. Tanium investigation capabilities now explicitly cover installed software, running processes, services, open ports, endpoint IP, and hardware/OS evidence. File/hash/domain indicators are preserved as IOC objects and unmatched indicators until dedicated read-only Tanium telemetry is available.
- 2026-08-05: Tanium Inventory page includes manual update and inventory-specific automatic update controls. Inventory automation uses a numeric interval plus unit selector (`minutes`, `hours`, `days`) and runs independently from CVE/News scheduled collection. Sidebar `SecureWatch` brand links to Dashboard home. Dashboard CVE and Security News items created within the last 24 hours show a green `NEW!` badge in the top-left of each item.
- 2026-08-06: `NEW!` badges now also appear on CVE and Security News list cards. A badge is shown only when the item was created within the last 24 hours and has not been marked read in browser `localStorage`. Clicking the Dashboard/list item or its source link marks it read, which removes the badge across Dashboard, CVE, and Security News on the same browser.
- 2026-08-06: Investigation now supports immediate read-only Tanium REST sensor questions for live endpoint checks. For kernel/Open vSwitch/file-path investigations, backend asks a question such as `Get Computer Name and Tanium Client IP Address and Kernel Modules and Kernel Version and File Exists[/etc/modprobe.d/ovswrap.conf] from all entities`, stores the rows under `investigation_runs.results.tanium_evidence.live_sensor_query`, and uses Kernel Version, Kernel Modules, and File Exists evidence to upgrade endpoint classification beyond stale Inventory snapshots.
