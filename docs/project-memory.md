...
- 2026-07-08: Settings source management no longer exposes the old broad `CVE Update Sources` section. It is replaced by `NVD Feed Sources`, showing only the `NVD JSON Feeds` source used by both `NVD Year Feed` and `최신 CVE Update`; CISA KEV, EPSS, and legacy NVD REST sources are hidden from that Settings source card.
