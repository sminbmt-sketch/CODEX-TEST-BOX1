...
- 2026-07-16: Tanium Inventory was extended beyond endpoint basics. Endpoint snapshots now store installed software, optional process sensor readings, service details, and SBOM package data in JSON fields. The inventory UI shows per-endpoint Software, Process, and SBOM buttons that open scrollable detail dialogs. Process values are split into individual rows, and SBOM packages are stored/displayed as Type, Name, and Version grouped by Type. Optional process/SBOM collection failures do not block the core endpoint/software sync.
- 2026-07-16: The current Tanium environment has an `SBOM Packages` sensor, but Gateway reported it is not registered with Tanium Data Service, so read-only endpoint sync currently returns no SBOM package rows. Registering/harvesting that sensor in TDS is required before SBOM Type/Name/Version data can populate.
