...
def apply_article_filters(query, q: str | None = None, category: str | None = None):
    if q:
        like = f"%{q}%"
        query = query.where(Article.title.ilike(like) | Article.summary.ilike(like) | Article.raw_excerpt.ilike(like))
    if category == "kisa":
        query = query.where(Article.source.has(Source.name.ilike("%KISA%")))
    elif category == "news":
        query = query.where(~Article.source.has(Source.name.ilike("%KISA%")))
    return query
...
def count_articles(
    q: str | None = None,
    category: str | None = Query(default=None, pattern="^(news|kisa)$"),
    db: Session = Depends(get_db),
) -> int:
    query = apply_article_filters(select(func.count(Article.id)), q=q, category=category)
...
def list_articles(
    ...
    query = apply_article_filters(select(Article).options(selectinload(Article.source)), q=q, category=category)
    if sort == "name":
